...
export function contentInsufficientBloods(day: number, value: number, getBloodsUserDB: number) {
    return `Você ainda não possui bloods suficientes. Você precisa conseguir mais \` ${
        value - getBloodsUserDB
    } Bloods 🩸\` para obter o cargo por ${day} dias.`;
}

export function contentAlreadyRole(getRoleActiveMember: string, getTimestampUserDB: number) {
    return `Hey, você já possui a assinatura de <@&${getRoleActiveMember}> em sua conta, ela irá acabar ${time(getTimestampUserDB, "F")}`;
}

export function contentLogStaff(userId: string, getTimestamp: number) {
    return `[LOG] O membro <@${userId}> obteve o cargo de **Membro Ativo** através da loja. Duração: ${time(getTimestamp, "F")}`;
}
